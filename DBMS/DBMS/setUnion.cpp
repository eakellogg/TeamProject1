...
static bool matchingData(map<string, vector<Attribute*>> firstTableData, vector<Attribute*> secondTableRow) {
	...
	for (map<string, vector<Attribute*>>::iterator i = firstTableData.begin(); i != firstTableData.end(); i++) {
		for (int j = 0; j < secondTableRow.size(); j++) {
...
Table* Engine::setUnion(Table* firstTable, Table* secondTable) {

	if (!unionCompatible(firstTable, secondTable)) {
		throw "tables are not union compatible, thus union cannot be performed";
	}
	...
	//add only the unique second table data to the new table
	for (map<string, vector<Attribute*>>::iterator i = secondTableData.begin(); i != secondTableData.end(); i++) {
		vector<Attribute*> newRow;
		secondTable->setCurrentRow(secondTableOffset);
		if (!matchingData(firstTableData, secondTable->getCurrentRow)){
			for (int i = 0; i < secondTable->getColumnTitles.size(); i++) {
				newRow.push_back(secondTable->getVariable(newColumnTitles[i]));
			}
			newTable->addRow(newRow, newColumnTypes);
		}
		firstTableOffset++;
